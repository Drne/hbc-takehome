...
import {ChatBox, ChatBubble, ChatFeed, ChatMeta, Input, Panel, PanelHeader, PanelTitle, PrimaryButton} from './common'

type ChatPanelProps = {
  currentEmployee: Employee
}

export function ChatPanel({currentEmployee}: ChatPanelProps) {
          ...
          {messages.length === 0 ? (
            ...
            messages.map((message) => (
              <ChatBubble key={message.id}>
                <ChatMeta>{message.employeeName} • {new Date(message.createdAt).toLocaleTimeString([], {hour: 'numeric', minute: '2-digit'})}</ChatMeta>
